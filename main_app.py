...
def generate_user_flows():
    """
    Generates a fully dynamic list of random user flows (150-250) for App traffic.
    ...
    for journey, bots in journey_bots.items():
        ...
        ret_pct = random.uniform(70, 75)
        sign_pct = random.uniform(5, 10)
        guest_pct = 100.0 - (ret_pct + sign_pct)
       
        returning_bots = int(round(bots * (ret_pct / 100.0)))
        signup_bots = int(round(bots * (sign_pct / 100.0)))
        guest_bots = bots - (returning_bots + signup_bots)
